...
### Certificate Transparency Intelligence & Network-Based Attribution

![crt.sh](https://img.shields.io/badge/crt.sh-Cert%20Transparency-2E8B57?style=for-the-badge&logo=letsencrypt&logoColor=white)
![Zeek](https://img.shields.io/badge/Zeek-Network%20Monitor-00A9E0?style=for-the-badge&logo=wireshark&logoColor=white)
...
- [🧰 Task 1: Install Required Tools and Setup Environment](#-task-1-install-required-tools-and-setup-environment)
- [📜 Task 2: Search and Retrieve Certificates from crt.sh](#-task-2-search-and-retrieve-certificates-from-crtsh)
- [📡 Task 3: Generate and Analyze Zeek Logs for TLS Connections](#-task-3-generate-and-analyze-zeek-logs-for-tls-connections)
- [🧩 Task 4: Identify Common Infrastructure Across Adversary Campaigns](#-task-4-identify-common-infrastructure-across-adversary-campaigns)
- [📊 Task 5: Generate Final Correlation Report](#-task-5-generate-final-correlation-report)
...
| # | Objective |
|---|-----------|
| 1 | Extract TLS certificate data from **crt.sh** certificate transparency logs |
| 2 | Analyze **Zeek** network logs to identify TLS connections and certificate fingerprints |
| 3 | Correlate certificate patterns across multiple adversary campaigns |
| 4 | Build infrastructure attribution maps using certificate commonalities |

## ✅ Prerequisites

| Requirement | Details |
|---|---|
| 🔐 TLS/PKI Basics | Basic understanding of TLS/SSL certificates and PKI concepts |
| 🐧 Linux CLI | Familiarity with Linux command line operations |
| 📡 Traffic Analysis | Knowledge of network traffic analysis fundamentals |
...
# TODO: Pin these package versions in a `requirements.txt` for reproducible lab runs
...
```python
#!/usr/bin/env python3
# 🔍 Query crt.sh certificate transparency logs for a target domain
import requests
import json
import sys
import time
...
```bash
# 🕵️ Search for certificates from known malicious domains (examples)
...
# ⏱️ Wait between requests to be respectful to crt.sh
sleep 5
```

# TODO: Replace the example domains with campaign indicators you're authorized to investigate

### 📊 Subtask 2.3: Create Certificate Analysis Script

```python
#!/usr/bin/env python3
# 📊 Analyze collected certificates for shared issuers, serials, and timing
import json
import os
...
```python
#!/usr/bin/env python3
# 🌐 Generate outbound TLS connections for Zeek to observe
import subprocess
import time
...
```bash
# ▶️ Start Zeek in background to monitor traffic
...
# TODO: Replace the demo domain list with real campaign-adjacent hostnames observed in your environment

### 🔬 Subtask 3.3: Analyze Zeek SSL/TLS Logs

```python
#!/usr/bin/env python3
# 🔬 Parse Zeek's ssl.log and correlate certs against crt.sh findings
import pandas as pd
import json
from collections import defaultdict
import os
...
### 🕸️ Subtask 4.1: Create Infrastructure Correlation Engine

```python
#!/usr/bin/env python3
# 🕸️ Build a graph of issuer/domain relationships and score attribution confidence
import json
import pandas as pd
...
def generate_attribution_report(patterns, data):
    ...
    # ♻️ Infrastructure reuse analysis
...
# TODO: Add a minimum-domains threshold parameter so low-signal issuers (e.g. shared free CAs) don't inflate confidence scores

### ▶️ Subtask 4.2: Run Complete Analysis Pipeline
...
```python
#!/usr/bin/env python3
# 📅 Visualize certificate issuance timing across campaign domains
import json
import pandas as pd
import matplotlib.pyplot as plt
...
def create_campaign_timeline():
    ...
    # 🖼️ Create timeline visualization
...
```python
#!/usr/bin/env python3
# 📝 Merge all findings into an executive summary, IOC list, and recommendations
import json
import os
from datetime import datetime
...
# TODO: Add a `--format html` option to `final_report.py` for a client-shareable report export
...
|---|---|---|---|
| T1596.002 | Search Open Technical Databases: WHOIS | Reconnaissance | crt.sh |
| T1596.001 | Search Open Technical Databases: DNS/Passive DNS | Reconnaissance | crt.sh |
| T1071.001 | Application Layer Protocol: Web Protocols | Command and Control | Zeek (ssl.log) |
| T1583.001 | Acquire Infrastructure: Domains | Resource Development | Infrastructure correlation engine |
| T1584.001 | Compromise Infrastructure: Domains | Resource Development | Certificate issuer/serial correlation |
...
<details>
<summary>❌ <code>zeek: command not found</code> or Zeek fails to start</summary>

Confirm Zeek installed correctly with `zeek --version`. If `zeek -i any` fails with a permissions error, re-run with `sudo` as shown in the lab, since packet capture requires elevated privileges.
</details>

<details>
<summary>❌ Empty or missing <code>zeek-logs/ssl.log</code></summary>

Ensure traffic was generated **while** Zeek was running in the background, and that `sudo cp /opt/zeek/logs/current/*.log zeek-logs/` ran before the Zeek process's log directory rotated.
</details>

<details>
<summary>❌ crt.sh requests time out or return empty results</summary>

crt.sh occasionally throttles or is briefly unavailable — the `cert_search.py` script already sets a 30-second timeout; wait and retry, and keep the `sleep 5` between queries to avoid rate limiting.
</details>

<details>
<summary>❌ <code>ModuleNotFoundError: No module named 'networkx'</code></summary>

The infrastructure correlation engine requires `networkx` in addition to the packages from Subtask 1.3:
...
- 🗺️ Built attribution maps connecting disparate campaigns through shared certificate infrastructure
- 📊 Generated actionable intelligence including IOCs, confidence assessments, and security recommendations

### 🌍 Real-World Applications

This methodology demonstrates how certificate transparency data combined with network monitoring can reveal adversary infrastructure reuse patterns, enabling proactive threat hunting and campaign attribution. The techniques learned here are essential for advanced threat intelligence analysis and can be applied to real-world incident response and threat hunting scenarios. The correlation approach mastered here helps security teams identify previously unknown malicious infrastructure and predict adversary expansion patterns, significantly enhancing organizational defensive capabilities.
